...
(define-public (batch-transfer (transfers (list 200
    {
    amount: uint,
    from: principal,
    to: principal,
    ...
    (fold transfer-many transfers (ok u0))
)

(define-private (transfer-many
        (transfer-data {
            amount: uint,
            from: principal,
            to: principal,
        ...
        (prior-result (response uint uint))
    )
    (match prior-result
        index (match (transfer (get amount transfer-data) (get from transfer-data)
            (get to transfer-data) (get memo transfer-data)
        )
            success (ok (+ index u1))
            error (err (+ ERR_TRANSFER_INDEX_PREFIX index))
        )
        error (err error)
    )
)
...
(define-read-only (get-total-supply-locked)
    (ok (ft-get-supply sbtc-token-locked))
)
