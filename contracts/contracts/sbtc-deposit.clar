...
(define-public (complete-deposit-wrapper
    ...
    (let (
        ...
        (contract-call? .sbtc-registry complete-deposit txid vout-index amount
            recipient burn-hash burn-height sweep-txid
        )
    )
)
...
(define-public (complete-deposits-wrapper (deposits (list 500
    {
    txid: (buff 32),
    vout-index: uint,
    amount: uint,
...
(define-private (complete-individual-deposits-helper
    ...
    (match helper-response
        index (begin
            (unwrap!
                (complete-deposit-wrapper (get txid deposit)
                    (get vout-index deposit) (get amount deposit)
                    (get recipient deposit) (get burn-hash deposit)
