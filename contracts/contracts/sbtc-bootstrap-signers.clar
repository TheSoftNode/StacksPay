...
(define-public (rotate-keys-wrapper
    ...
    (let ((new-signer-principal (pubkeys-to-principal new-keys new-signature-threshold)))
        ...
        (asserts!
            (and
                (> new-signature-threshold (/ (len new-keys) u2))
                (<= new-signature-threshold (len new-keys))
            )
        ...
        (asserts!
            (is-eq (contract-call? .sbtc-registry get-current-signer-principal)
                tx-sender
            )
            ERR_INVALID_CALLER
        )
        ...
        (contract-call? .sbtc-registry rotate-keys new-keys new-signer-principal
            new-aggregate-pubkey new-signature-threshold
        )
    )
)
...
(define-public (update-protocol-contract-wrapper
    ...
    (begin
        ...
        (contract-call? .sbtc-registry update-protocol-contract contract-type
            contract-address
        )
    )
)
...
(define-private (signer-key-length-check
        (current-key (buff 33))
        (helper-response (response uint uint))
    )
    (match helper-response
        index (begin
            (asserts! (is-eq (len current-key) key-size)
                (err (+ ERR_KEY_SIZE_PREFIX (+ u10 index)))
            )
            (ok (+ index u1))
        )
...
(define-read-only (concat-pubkeys-fold
        (pubkey (buff 33))
        (iterator (buff 4352))
    )
    (let (
