...
import { CoinbaseCharge, CoinbaseCheckout, CoinbaseCommerceConfig, CoinbaseWebhookEvent } from '@/interfaces/coinbase.interface';
...
export class CoinbaseCommerceService {
  ...
  private async makeRequest(
    ...
    if (!response.ok) {
      ...
      throw new Error(`Coinbase Commerce API error: ${response.status} - ${(errorData as any).error?.message || (errorData as any).message || response.statusText}`);
    }
